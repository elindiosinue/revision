...
NOSQL = False  # Variable global
DESENCRIPTAR = True  # Variable global
...
def set_desencriptar(value):
    global DESENCRIPTAR
    DESENCRIPTAR = value

def set_fichero(value):
    global FICHERO
    FICHERO = value
...
def descrypt(fichero, dir):
    ...
    Args:
        fichero (str): Nombre del archivo a organizar.
        dir (str): Directorio de procesamiento.
    """
    try:
        # Validación inicial
        if not fichero or not re.search(r'\.pdf$', fichero, re.IGNORECASE):
            raise ValueError(f"Archivo no válido o no es PDF")
        
        if not dir:
            raise ValueError(f"Directorio no válido: {dir}")
            ...
            with conn.cursor() as cur:
                ...
                for row in rows:
                    try:
                        ...
                        #logs_datos(f"descrypt {archivo_salida} desencriptado exitosamente.", 'I')
                        return nombre_salida
                    except (pikepdf.PasswordError) as error:
                        l_error = True
                        continue
                if l_error:
                    raise ValueError(f"Todas las Contraseñas son incorrectas.")
    except (Exception, psycopg2.DatabaseError) as error:
        logs_datos(f'Error al desencriptar fichero: {fichero} # error: {error}')
    finally:
        if conn is not None:
            conn.close()
...
def pdfToText(fichero, dir):
    """
    Pasar la información de un fichero RNT en texto plano a un CSV.
    ...
    try:
        ...
        logs_datos(f'Error el desencriptar el fichero: {fichero} # error: {error}')
        return None

def txtToCsvRNT(fichero, dir):
    """
    Organizar fichero.
    ...
    try:
        # Validación inicial
        if not fichero or not re.search(r'\.pdf$', fichero, re.IGNORECASE):
            raise ValueError(f"Archivo no válido o no es PDF: {fichero}")
        ...
        # creating a pdf reader object 
        #print(f"txtToCsvRNT({fichero}, {dir})")
        if os.path.exists(archivo_entrada):
            #print(f"txtToCsvRNT fichero: {fichero}")
            try:
                ...
                try:
                    ...
                    raise ValueError(f'No se encuentra centro fichero: {fichero} # error: {error}')
                finally:
                    if conn is not None:
                        conn.close()
                ...
                raw_values = {
                    ...
                    "empresa_id": empresa_id,
                    "centro_id": centro_id,
                    "anno_id": periodo_desde.split('/')[1],
                    "mes_id": periodo_desde.split('/')[0],
                }
                ...
                raise ValueError(f'Error al obtener los datos para el CSV, fichero: {fichero}, ERROR: {error}')
        else:
            raise ValueError(f"NO EXISTE {fichero}")
    except Exception as e:
        logs_datos(f"RNT: Error al pasar el PDF a CSV: {e}")

def csvToSQLRNT(fichero, dir):
    """
    Pasar la información de un RNT en un fichero CSV a Base de Datos
    ...
    try:
        # Validación inicial
        if not fichero or not re.search(r'\.csv$', fichero, re.IGNORECASE):
            raise ValueError(f"Archivo no válido o no es PDF: {fichero}")
                ...
                for row in reader:
                    ...
                    if rnts_resumen_id is None or row['rnts_resumen_id'] != rnts_resumen_id:
                        ...
                        try:
                            ...
                            raise ValueError(f'Error al obtener el Resumen, fichero: {fichero} # error: {error}')
                        finally:
                            if conn is not None:
                                conn.close()
                        if resumen is not None:
                            ...
                            raise ValueError(f"Error al obtener el Resumen, fichero: {fichero}")
                    ...
                    try:
                        with connect() as conn:
                            with conn.cursor() as cur:
                                ...
                                #print(f"csvToSQLRNT Trabajador sql: {sql}")
                                cur.execute(sql)
                                row_trabajador = cur.fetchone()
                                if row_trabajador and len(row_trabajador) > 0:
                                    trabajador_id = row_trabajador[0]
                    except (Exception, psycopg2.DatabaseError) as error:
                        logs_datos(f'csvToSQLRNT Error al obtener el trabajador, fichero: {fichero} # error: {error}', 'W')
                    finally:
                        if conn is not None:
                            conn.close()
                    ...
                    try:
                        with connect() as conn:
                            with conn.cursor() as cur:
                                ...
                                #logs_datos(f"csvToSQLRNT Tipo de Complemento sql: {sql}")
                                cur.execute(sql)
                                ...
                                if rows_complementos_tipo and len(rows_complementos_tipo) > 0:
                                    ...
                                    logs_datos(f"Insertado Tipo de Complemento con ID: {complementos_tipo_id}", 'I')

                    except (Exception, psycopg2.DatabaseError) as error:
                        logs_datos(f'csvToSQLRNT Error al obtener el Tipo de Complemento, fichero: {fichero} # error: {error}', 'W')
                    finally:
                        if conn is not None:
                            conn.close()
                    ...
                    i += 1
                logs_datos(f"TODO OK; errors: {num_errors}", 'I')
    except Exception as e:
        logs_datos(f"Error al cargar el CSV a la Base de Datos: {e}")

def txtToCsvRLC(fichero, dir):
    """
    Pasar la información de un fichero RLC en texto plano a un CSV.
    ...
    try:
        # Validación inicial
        if not fichero or not re.search(r'\.txt$', fichero, re.IGNORECASE):
            raise ValueError(f"Archivo no válido o no es TXT: {fichero}")
        ...
        pagina = None
        periodo_pago = None
        periodo_pago_desde = None
        periodo_pago_hasta = None
        num_emisora = None
        num_referencia = None
        identificacion = None
        importe = None
        liquido_totales = None
        

        # creating a pdf reader object 
        #logs_datos(f"txtToCsvRNT({fichero}, {dir})", 'I')
        if os.path.exists(archivo_entrada):
            #logs_datos(f"txtToCsvRNT fichero: {fichero}", 'I')
            try:
                with open(archivo_entrada) as f:
                    csv_line = []
                    datos_referencia = False
                ...
                try:
                    ...
                    print(f'txtToCsvRNT fichero: {fichero} # error: {error}')
                finally:
                    if conn is not None:
                        conn.close()
        ...
        logs_datos(f"Error al pasar de PDF a CSV: {e}")

def csvToSQLRLC(fichero, dir):
    """
    Pasar la información de un fichero RLC en un CSV a Base de Datos.
    ...
    try:
        # Validación inicial
        if not fichero or not re.search(r'\.csv$', fichero, re.IGNORECASE):
            raise ValueError(f"Archivo no válido o no es CSV: {fichero}")
                ...
                for row in reader:
                    ...
                    if rnts_resumen_id is None or row['rnts_resumen_id'] != rnts_resumen_id:
                        ...
                        try:
                            ...
                            raise ValueError(f'csvToSQLRNT Error al obtener el resumen: {fichero} # error: {error}')
                        finally:
                            if conn is not None:
                                conn.close()
                        if resumen is not None:
                            ...
                            raise ValueError(f'csvToSQLRNT Error al obtener el resumen: {fichero}')
                    ...
                    try:
                        ...
                        logs_datos(f'csvToSQLRLC: Error al obtener al Trabajador fichero: {fichero} # error: {error}')
                    finally:
                        if conn is not None:
                            conn.close()
                    ...
                    try:
                        with connect() as conn:
                            with conn.cursor() as cur:
                                ct_table_name = 'complementos_tipos'
                                sql = f"select id from {ct_table_name} where sinacentos(descripcion) = sinacentos('{complementos_tipo}')"
                                #logs_datos(f"csvToSQLRLC: {ct_table_name} sql: {sql}", "I")
                                cur.execute(sql)
                                rows_complementos_tipo = cur.fetchone()
                                if rows_complementos_tipo and len(rows_complementos_tipo) > 0:
                                    complementos_tipo_id = rows_complementos_tipo[0]
                        ...
                        logs_datos(f'csvToSQLRLC: Error al obtener el Tipo de complemento fichero: {fichero} # error: {error}')
                    finally:
                        if conn is not None:
                            conn.close()
...
def organizar(fichero, dir):
    ...
    try:
        # Validación inicial
        if not fichero:
            raise ValueError(f"Archivo no válido: {fichero}")
        ...
        if os.path.exists(archivo_pdf):
            ...
            logs_datos(f"Organizar: No se ha encontrado el fichero pdf: {archivo_pdf}", 'W')
        ...
        if os.path.exists(archivo_csv):
            ...
            logs_datos(f"Organizar: No se ha encontrado el fichero csv: {archivo_csv}", 'W')
            
    except Exception as e:
        logs_datos(f"Error al organizar los ficheros: {e}")

def procesar_rnt(texto_procesado, dir):
    """
    Procesa documentos de tipo RNT.
    
    Args:
        texto_procesado (str): Nombre del archivo TXT.
        dir (str): Directorio de procesamiento.
    """
    try:
        # Validación inicial
        if not texto_procesado:
            raise ValueError(f"Archivo no válido: {texto_procesado}")
        
        if not dir:
            raise ValueError(f"Directorio no válido: {dir}")
        
        csv_rnt = txtToCsvRNT(texto_procesado, dir)
        if not csv_rnt:
            raise ValueError("Fallo al generar CSV para RNT")
...
def procesar_rlc(texto_procesado, dir):
    """
    Procesa documentos de tipo RLC.
    ...
    try:
        ...
        csv_rlc = txtToCsvRLC(texto_procesado, dir)
        if not csv_rlc:
            raise ValueError("Fallo al generar CSV para RLC")
    ...
    try:
        ...
        try:
            ...
            if DESENCRIPTAR:
                ...
                fichero_descifrado = fichero_renombrado
            ...
            raise ValueError(f"Error en etapa de preprocesamiento: {e}")
        ...
        if re.search(r'RNT', texto_procesado):
            ...
            raise ValueError(f"Documento sin tipo definido (RNT/RLC): {fichero}")

        if not resultado:
            raise ValueError(f"Fallo al procesar documento: {fichero}")
            
    except Exception as e:
        logs_datos(f"Error en main: {e}")
...
def recorreDirectorio(dir):
    ...
    try:
        ...
        logs_datos(f"Error en recorreDirectorio: {e}")
...
if __name__ == '__main__':
    ...
    if args.desencriptar == 'N':
        desencriptar=False
    set_desencriptar(desencriptar)

    if args.ruta == "ROOTDIR":
        recorreDirectorio(ROOTDIR)
    if isdir(args.ruta):
        recorreDirectorio(args.ruta)
    elif isfile(args.ruta):
        main(args.ruta)
    else:
        logs_datos(f"ROOT: Error: La ruta {args.ruta} especificada no es válida.")
        sys.exit(1)